...
def getSaveDir(model, optimiser):
  directory = os.path.dirname(os.path.abspath(__file__))
  if "Windows" in platform.system():
    directory="/".join(directory.split("\\"))
  directory=directory.rsplit("/",1)[0] + "/Results/" + model + "/" + optimiser + "/"
...
def openTensorBoardAtIndex(model,optimiser,ind,portTrain=8001,portTest=8002):
  directory = os.path.dirname(os.path.abspath(__file__))
  if "Windows" in platform.system():
    directory="/".join(directory.split("\\"))
  directory=directory.rsplit("/",1)[0] + "/Results/" + model + "/" + optimiser + "/"
