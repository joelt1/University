...
int main(int argc, char *argv[]) {
	...
	*/
	...
	int M;
	// Number of threads along vertical axis in a grid block within a 2D CUDA thread grid
	// Used in inialising S_(0) and computing S_(n+1) at every timestep n
	int vertical_threads;

	if (argc > 1) {
		D = atoi(argv[1]);
		N = atoi(argv[2]);
		M = atoi(argv[3]);
		vertical_threads = atoi(argv[4]);
	} else {
		D = 8;
		N = 100;
		M = 10000;
	...
	perform_mc_simulation(D, N, M, K, r, T, vertical_threads);
...
void perform_mc_simulation(const int D, const int N, const int M, const int K, const float r, const int T,
	...
	*/
    ...
    dim3 dim_block(D, vertical_threads);  // Total of D*vertical_threads threads per block
