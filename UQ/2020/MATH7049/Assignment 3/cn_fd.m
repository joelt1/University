% Solves the untransformed B-S PDE for the price of an asset-or-nothing
% call option using the Crank-Nicolson finite difference scheme.
clear all;
format long
...
T = 2;          % Time to expiry
theta = 0.5;    % 0: explicit, 1 for implicit, and 0.5 for CN
...
for nn = 1:length(N_list)
    ...
    dx = (log(S(end - 1)) - log(S(2)))/m;
    
    % Store these for potential analysis
    alpha(nn) = 1/2 * sigma^2 * (dt/dx^2);
    lambda_cn(nn) = (1 - 2*alpha(nn))/(1 + 2*alpha(nn));
    lambda_imp(nn) = 1/(1 + 4*alpha(nn));
    ...
    for n = 1:N
        ...
        % Use linearity boundary conditions from Q2.a):
