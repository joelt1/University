...
% call option using Rannacher smoothing.
clear all;
format long
...
T = 2;          % Time to expiry
theta = 1;      % Initially 1 for implicit
...
for nn = 1:length(N_list)
    ...
    % Loop over timestep
    for n = 1:N
        % Switch to Crank-Nicolson scheme for all remaining time steps
        if n > 3
            theta = 0.5;
        end
